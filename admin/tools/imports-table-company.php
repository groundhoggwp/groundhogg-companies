...
class Imports_Table_Company extends WP_List_Table {
	...
	public function __construct() {
		...
		parent::__construct( array(
			'singular' => 'import_company',     // Singular name of the listed records.
			'plural'   => 'imports_companies',    // Plural name of the listed records.
	...
	function prepare_items() {
        ...
        if ( file_exists(get_company_imports_dir() ) )  {

            $scanned_directory = array_diff(scandir( get_company_imports_dir() ), ['..', '.']);

            foreach ($scanned_directory as $filename) {

                $filepath = get_company_imports_dir( $filename );

                $file = [
                    'file' => $filename,
                    'file_path' => $filepath,
                    'file_url' => file_access_url( '/company-imports/' . $filename, true ),
	...
	protected function handle_row_actions( $import, $column_name, $primary ) {
		...
		$actions['import'] = sprintf(
			...
			/* translators: %s: title */
			admin_url( 'admin.php?page=gh_tools&tab=import_companies&action=map&import=' . $import[ 'file' ] ),
			esc_attr( 'Import' ),
			__( 'Import' )
		);
		...
		$actions['delete'] = sprintf(
			...
			wp_nonce_url(admin_url( 'admin.php?page=gh_tools&tab=import_companies&action=delete&import=' . $import[ 'file' ] )),
			/* translators: %s: title */
